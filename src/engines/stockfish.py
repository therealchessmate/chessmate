...
class Stockfish:
    ...
    def get_evaluation_info(self) -> str:
        ...
        Returns:
            A string with detailed evaluation terms (material, pawns, mobility, etc.)
        """
        self._put("eval")
        output_lines = []
        while True:
            line = self._read_line()
            # The eval output ends with a blank line or "readyok"
            if not line or line.startswith("bestmove") or line == "readyok":
                break
            output_lines.append(line)
            # Stop if we hit the "Final evaluation" line from Eval::trace
